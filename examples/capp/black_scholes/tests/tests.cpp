...
string kSource   = "BlackScholes.cl";
string kName     = "BlackScholes";
...
int main(int argc, char* argv[]) {
	...
	BlackScholes bsParallel(cpuDevice, kSource, kName);

	// Run kernel (one function call once data is ready)
	bsParallel.runKernel(in, output);

	// Print the runtime 
	cout << NUM_OPTIONS << " " <<
			bsParallel.GetRunTime().tv_nsec << endl;
}
